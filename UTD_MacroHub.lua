...
Events=al:AddTab({Title="Events",Icon="shield"}),
...
local function _isGuidLike(aP)
return type(aP)=="string"and#aP>=8 and aP:match("^[%w%-]+$")~=nil
end

local function _firePromptsNear(aP,aQ)
if type(fireproximityprompt)~="function"then return 0 end
local aR=0
for aS,aT in ipairs(workspace:GetDescendants())do
if aT:IsA("ProximityPrompt")and aT.Enabled then
local aU=aT.Parent
local aV
if aU and aU:IsA("BasePart")then
aV=aU.Position
elseif aU and aU:IsA("Model")then
local aW,aX=pcall(function()return aU:GetPivot()end)
if aW then aV=aX.Position end
elseif aU and aU:IsA("Attachment")then
aV=aU.WorldPosition
end
if aV and(aV-aP).Magnitude<=aQ then
pcall(fireproximityprompt,aT)
aR=aR+1
end
end
end
...
local function _tpFireAndReturn(aP)
local aQ=e.Character
local aR=aQ and aQ:FindFirstChild("HumanoidRootPart")
local aS
if aR then
aS=aR.CFrame
pcall(function()aR.CFrame=CFrame.new(aP.Position+Vector3.new(0,3,0))end)
task.wait(0.1)
end
local aT=_firePromptsNear(aP.Position,30)
if aR and aS then
task.wait(0.05)
pcall(function()aR.CFrame=aS end)
end
return aT
end

local function _refreshMochiStatus()
if not aN then return end
local aP=aM.mochiAuto and"ON"or"OFF"
aN:SetDesc(("Status: %s  |  Units freed: %d"):format(aP,aM.mochiFreed))
end

local function _stopMochiWatcher()
for aP,aQ in ipairs(aM.mochiConns)do
pcall(function()aQ:Disconnect()end)
end
aM.mochiConns={}
end

local function _freeFromMochi(aP)
if not aM.mochiAuto then return end
if not(aP and aP.Parent and aP.Parent:IsA("BasePart"))then return end

local aQ=aP.Parent
local aR=e.Character
local aS=aR and aR:FindFirstChild("HumanoidRootPart")
local aT=aS and aS.CFrame

if aS then
pcall(function()aS.CFrame=CFrame.new(aQ.Position+Vector3.new(0,3,0))end)
task.wait(0.15)
end
...
local aU=tick()+3
local aV=false
while tick()<aU and aM.mochiAuto do
if not(aP and aP.Parent)then aV=true;break end
pcall(fireproximityprompt,aP)
task.wait(0.2)
end

if aS and aT then
pcall(function()aS.CFrame=aT end)
end

if aV then
aM.mochiFreed=aM.mochiFreed+1
_refreshMochiStatus()
end
end

local function _startMochiWatcher()
_stopMochiWatcher()
if not aM.mochiAuto then return end
if type(fireproximityprompt)~="function"then return end


for aP,aQ in ipairs(workspace:GetDescendants())do
if aQ:IsA("ProximityPrompt")and aQ.Name=="FreeMochiUnitPrompt"then
task.spawn(_freeFromMochi,aQ)
end
end


local aP=workspace.DescendantAdded:Connect(function(aP)
if not aM.mochiAuto then return end
if aP:IsA("ProximityPrompt")and aP.Name=="FreeMochiUnitPrompt"then

task.wait(0.05)
_freeFromMochi(aP)
end
end)
table.insert(aM.mochiConns,aP)
end

aI.Events:AddSection("INFO")
aI.Events:AddParagraph({
Title="About",
Content="Auto-handlers for in-game event mechanics. Toggle a feature here, then run the game normally — the script intervenes when the mechanic triggers.",
})

aI.Events:AddSection("MOCHI (FREEZE)")
aI.Events:AddParagraph({
Title="What it does",
Content="Mochi freezes one of your units and requires you to walk over and hold E on a ProximityPrompt to free it. With this on, the script auto-fires the nearby ProximityPrompt the moment a freeze effect spawns.",
})

aN=aI.Events:AddParagraph({Title="Mochi",Content="Status: OFF  |  Units freed: 0"})

aO=aI.Events:AddToggle("MochiAutoFree",{
Title="Auto-Free from Mochi",
Default=aM.mochiAuto,
Callback=function(aP)
aM.mochiAuto=aP
F.mochiAuto=aP
w.SaveSettings(F)
if aP then _startMochiWatcher()else _stopMochiWatcher()end
_refreshMochiStatus()
end,
})
...
aI.Events:AddButton({
Title="Reset Counter",
Callback=function()aM.mochiFreed=0;_refreshMochiStatus()end,
})


if aM.mochiAuto then _startMochiWatcher()end


aI.Settings:AddSection("GAME SETTINGS")
aI.Settings:AddParagraph({
Title="Note",
Content="AutoStart & AutoRestart are forced OFF only while Auto Farm is running. They stay untouched during normal play.",
})
...
Rounding=2,
Callback=function(aP)F.sellDelay=aP;w.SaveSettings(F)end,
})
...
Rounding=0,
Callback=function(aP)F.scanRadius=aP;w.SaveSettings(F)end,
})
...
Rounding=0,
Callback=function(aP)F.lobbyWait=aP;w.SaveSettings(F)end,
})
...
Rounding=0,
Callback=function(aP)F.replayWait=aP;w.SaveSettings(F)end,
})
...
if aP then aP:Disconnect();aP=nil end
if aQ then
aP=e.Idled:Connect(function()
d:CaptureController()
...
if not aQ then t:Notify({Title="Dex",Content="Failed to load — link may be outdated.",Duration=3})end
end,
})
...
if not aQ then t:Notify({Title="Remote Spy",Content="Failed to load — link may be outdated.",Duration=3})end
end,
})
...
t:Notify({Title="UTD Hub",Content="Loaded.",Duration=3})
